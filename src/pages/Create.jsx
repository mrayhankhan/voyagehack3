...
const Create = () => {
  ...
  // Handle Progress Ring calculation
  useEffect(() => {
    setPercent(Math.round((step / (STEPS.length - 1)) * 100));
  }, [step]);

  // Handle animation per step
  useEffect(() => {
    if (panelRef.current) {
  ...
  const simulateTerminal = (logs, onComplete) => {
  ...
  const handleNext = () => {
    if (step === 4 && formData.budget > 0) { // Destination Selection AI
       simulateTerminal([
    ...
    if (step === 6) { // Lock Inventory 
      simulateTerminal([
        `> Checking ${formData.destination || 'Taj Lake Palace'} availability...`,
        `> Locking ${Math.ceil(formData.headcount / 2)} rooms...`,
  ...
  const handleLaunch = () => {
    ...
    navigate(`/microsite/${id}`);
  };

  const isStepValid = () => {
    if (step === 0) return formData.brideName && formData.groomName && formData.contact;
    if (step === 1) return formData.budget > 0;
    if (step === 2) return formData.headcount > 0 && formData.dates;
    if (step === 3) return formData.timings.length > 0;
    if (step === 4) return formData.destination;
    return true; // other steps naturally proceed
  };
          ...
          <div className="space-y-4">
            {STEPS.map((s, i) => (
              <div key={s} className="flex items-center gap-4">
                ...
                <div className={`font-sans font-semibold text-sm transition-colors ${i <= step ? 'text-tbo-indigo' : 'text-gray-400'}`}>
                  {s}
                </div>
              </div>
            ))}
          ...
          <div ref={panelRef} className="flex-1">
              ...
              <div className="space-y-6">
                ...
                <div className="p-5 bg-tbo-indigo/5 rounded-2xl border border-tbo-indigo/10 flex items-center justify-between mt-6">
                  ...
                  <span className="font-mono font-bold text-2xl text-tbo-emerald">
                    ${formData.headcount > 0 ? Math.round(formData.budget / formData.headcount).toLocaleString() : 0}
                  </span>
               </div>
              </div>
            )}
            ...
            {step === 4 && (
              <div className="space-y-6 h-full flex flex-col">
                ...
                {isProcessing ? (
                  <div className="flex-1 bg-tbo-indigo rounded-2xl p-6 font-mono text-sm text-tbo-emerald flex flex-col shadow-inner">
                    <div className="flex items-center gap-2 mb-4">
                       <div className="w-2 h-2 rounded-full bg-tbo-emerald animate-ping opacity-75"></div>
                       <span className="text-white text-xs uppercase tracking-widest">WSS Connection Active</span>
                    </div>
                    {terminalText.map((txt, idx) => (
                      <div key={idx} className="mb-2 opacity-90">&gt; {txt}</div>
                    ))}
                    <div className="animate-pulse bg-tbo-emerald w-2 h-4 mt-1"></div>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {['Udaipur, Rajasthan', 'Jaipur, Rajasthan', 'Goa, India'].map((dest) => (
                      <div 
                        key={dest} 
                        onClick={() => setFormData({...formData, destination: dest})}
                        className={`cursor-pointer border-2 rounded-2xl p-6 transition-all ${
                          formData.destination === dest ? 'border-tbo-saffron bg-tbo-saffron/5 shadow-md' : 'border-gray-100 hover:border-gray-300'
                        }`}
                      >
            ...
            {step === 6 && (
              <div className="space-y-6 h-full flex flex-col">
                ...
                {isProcessing ? (
                  <div className="flex-1 bg-tbo-indigo rounded-2xl p-6 font-mono text-sm text-tbo-rani flex flex-col shadow-inner">
                    <div className="flex items-center gap-2 mb-4">
                       <div className="w-2 h-2 rounded-full bg-tbo-rani animate-ping opacity-75"></div>
                       <span className="text-white text-xs uppercase tracking-widest">Inventory Extranet</span>
                    </div>
                    {terminalText.map((txt, idx) => (
                      <div key={idx} className="mb-2 opacity-90">&gt; {txt}</div>
                    ))}
                     <div className="animate-pulse bg-tbo-rani w-2 h-4 mt-1"></div>
                  </div>
                ) : (
            ...
            {step === 7 && (
              <div className="space-y-6 text-center py-8">
                 ...
                 <div className="inline-block bg-gray-50 border border-gray-200 rounded-2xl p-6 mb-8 w-full max-w-sm">
                   ...
                   <div className="font-mono text-tbo-indigo font-bold bg-white border border-gray-100 p-2 rounded-lg">{(Math.random() * 100000000).toString(16).toUpperCase()}</div>
                 </div>
              </div>
            )}
          </div>
